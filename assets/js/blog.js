...
function renderStudents() {
  teknoloji.innerHTML = '<h1>teknoloji</h1>';
  korku.innerHTML = '<h1>korku</h1>';
  saglik.innerHTML = '<h1>saglik</h1>';
  gezi.innerHTML = '<h1>gezi</h1>';
  yemek.innerHTML = '<h1>yemek</h1>';

  for (let i = 0; i < studentList.length; i++) {

    if (studentList[i].category === "teknoloji") {
        teknoloji.innerHTML += `<p>${studentList[i].name} - ${studentList[i].aciklama}</p>`;
    } else if (studentList[i].category === "korku") {
        korku.innerHTML += `<p>${studentList[i].name} - ${studentList[i].aciklama}</p>`;
    } else if (studentList[i].category === "saglik") {
        saglik.innerHTML += `<p>${studentList[i].name} - ${studentList[i].aciklama}</p>`;
    } else if (studentList[i].category === "gezi") {
        gezi.innerHTML += `<p>${studentList[i].name} - ${studentList[i].aciklama}</p>`;
    } else if (studentList[i].category === "yemek") {
        yemek.innerHTML += `<p>${studentList[i].name} - ${studentList[i].aciklama}</p>`;
    }
}
